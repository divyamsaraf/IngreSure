...
Task 1: plant_mod + dairy_head (+ passthrough). Culinary/process/Tier-1 in Task 2.
"""
...
def apply_policies(
    ...
    *,
    ...
    Conflict lock (LCT-1): plant_mod is checked and returned before dairy_head.
    """
    raw = (phrase or "").strip()
    if not raw:
    ...
    # --- LCT-1 conflict block: plant_mod BEFORE dairy_head ---
    for i in range(len(tokens) - 1):
        a, b = tokens[i], tokens[i + 1]
        # plant_mod adjacent to animalish / dairy_head partner (either order)
