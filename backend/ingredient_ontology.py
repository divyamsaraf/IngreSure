...
def evaluate_ingredient_risk(ingredient: str, profile: UserProfile) -> Dict:
    ...
    normalized_profile_allergens = set()
    for a in profile.allergens:
        lower_a = a.lower()
        normalized_profile_allergens.add(lower_a)
        if lower_a in allergy_map:
            normalized_profile_allergens.add(allergy_map[lower_a])
    
    for allergen in prop.allergens:
        if allergen in normalized_profile_allergens:
